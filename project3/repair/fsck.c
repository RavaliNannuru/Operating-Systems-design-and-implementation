...
char **argv;
{
	...
	while(TRUE){
		...
		char file_path[128];
		printf("\nSelect an option\n");
		printf("1. repair Inode, Zone map\n");
		printf("2. repair Inode of a file\n");
		printf("0. Break\n");
		scanf("%d", &choice);
		getchar();
		switch(choice){
		case 1:
			...
			printf("Enter Mount File to check:");
			scanf("%s", file_path);
			repair ^= 1;
			chkdev(file_path, clist, ilist, zlist);
			clist = 0;
			ilist = 0;
			zlist = 0;
			devgiven = 1;
			break;
		case 2:
			printf("\nFrom Repair Inode of a file\n");
			printf("Enter File to check for its inode:");
			scanf("%s", file_path);
