...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_MQ_OPEN)	= do_mq_open,            /* mq_open() */
	CALL(PM_MQ_CLOSE)	= do_mq_close,			/* mq_close() */
	CALL(PM_MQ_SEND)	= do_mq_send,			/* mq_send() */
	CALL(PM_MQ_RECEIVE)	= do_mq_receive,		/* mq_receive() */
	CALL(PM_MQ_SETATTR)	= do_mq_setattr,		/* mq_setattr() */
	CALL(PM_MQ_GETATTR)	= do_mq_getattr,		/* mq_getattr() */
	CALL(PM_MQ_REQNOTIFY) = do_mq_reqnotify
};
