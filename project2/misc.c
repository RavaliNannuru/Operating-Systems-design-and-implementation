...
int do_mq_open() {
	...
	if (start == 0) {
		...
		//set size of 150 to message_queue
		message_queue = malloc(151 * sizeof(char*));
		int i=0;
		for(;i<150;i++) {
			message_queue[i]=(char *)malloc(30*sizeof(char));
	...
	//check whether queue already exist. If queue exist get the queue index
	for(int i = 0;i<10;i++) {
	...
	//check whether we can create new queue
	for(int i=0;i<10;i++) {
		if(inuse[i]==0){
			queue_available = 1;
			new_queue_position=i;
			break;
		}
	}
